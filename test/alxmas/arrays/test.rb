...
class Alxmas::ArraysTest < Minitest::Test
  ...
  def test_bin_search
    ...
    assert Alxmas::Arrays.search([1, 4, 5, 7, 8, 9], 9) == 5
    assert Alxmas::Arrays.search([1, 4, 5, 7, 8, 9], 1) == 0
    assert Alxmas::Arrays.search([1, 4, 5, 7, 8, 9], 6) == -1

    array = (1..10000).to_a
    assert Alxmas::Arrays.search(array, array[1000]) == 1000
  end
end
